...
const SUGGESTIONS = [
  'Ce véhicule est-il fiable ?',
  'Quels sont les points à vérifier ?',
  'Ce prix est-il correct ?',
]

export default function ChatPanel({ analysisId, vehiculeLabel, open, onClose }) {
  ...
  useEffect(() => {
    if (!open || !analysisId || !session || historyLoaded) return

    fetch(`${API_URL}/api/chat/${analysisId}/`, {
  ...
  }, [open, analysisId, session, historyLoaded])
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/chat/`, {
        ...
        body: JSON.stringify({ analysis_id: analysisId, message: userMessage, history }),
      })
  ...
  return createPortal(
    <>
      <div className="fixed inset-0 bg-black/25 z-[55] no-print" onClick={onClose} />

      <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[420px] bg-card border-l border-border z-[60] flex flex-col no-print shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-4 h-[52px] border-b border-border shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              {vehiculeLabel && (
                <p className="text-[11px] text-text-tertiary truncate max-w-[270px]">{vehiculeLabel}</p>
              )}
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          ...
          {historyLoaded && messages.length === 0 && (
            <div className="text-center py-10 space-y-3">
              ...
              <p className="text-sm text-text-secondary">Posez vos questions sur ce véhicule</p>
        ...
        <div className="px-4 py-3 border-t border-border shrink-0">
          <div className="flex gap-2 items-end">
            <textarea
              ...
              placeholder="Posez une question… (Shift+Entrée pour un saut de ligne)"
